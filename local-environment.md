...
This guide describes the necessary steps to deploy Fortishield on a local Kubernetes environment (Microk8s, Minikube, Kind).
...
You can generate self-signed certificates for the ODFE cluster using the script at `fortishield/certs/indexer_cluster/generate_certs.sh` or provide your own.

Since Dashboard has HTTPS enabled it will require its own certificates, these may be generated with: `openssl req -x509 -batch -nodes -days 365 -newkey rsa:2048 -keyout key.pem -out cert.pem`, there is an utility script at `fortishield/certs/dashboard_http/generate_certs.sh` to help with this.
